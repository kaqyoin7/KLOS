...
LABEL_DESC_TEST:     Descriptor       0500000H,           0FFFFH,    DA_DRW     ; 用于访问保护模式下 5MB 内存处数据
...
; ------------------------------------------------------------------------
; 读取 ES 段（内存5MB处）开始的 8 字节，依次输出每个字节的十六进制编码，再换行
TestRead:
    xor esi, esi
    mov ecx, 8
.loop:
...
; ------------------------------------------------------------------------
; 将一字节AL的16进制数以字符串形式输出到屏幕（例：9CH -> "9""C"）
DispAL:
    push ecx
    push ebx
    ...
    mov bl, al
    shr al, 4
    mov ecx, 2  ; 循环两次，第一次将高4位的16进制转为字符串，第二次将低4位的16进制转为字符串
.loop:
    and al, 0fh
    cmp al, 9
    ja  .1
